...
void handle_mess (int num_new_mess)
{
  ...
  for (int i = 0; i <num_new_mess; i++)
  {
    ...
    if (t < 20)
    {
      tgbot.sendMessage(chat_id, "Дома холодно. Температура: " + String(t,1) + " градусов Цельсия." + " Включите подогрев пола или закройте окна.");
    }
    if (h >= 50)
    {
      tgbot.sendMessage(chat_id, "Дома высокая влажность: " + String(h,1) + " %" + " Проветрите квартиру.");
    }
    if (t >= 20)
    {
    ...
    if (h < 50)
    {
      tgbot.sendMessage(chat_id, "Дома нормальная влажность: " + String(h,1) + " %." + "Можете заканчивать проветривание.");
    }
  }
}
